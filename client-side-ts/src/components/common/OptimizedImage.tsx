...
export const OptimizedImage = ({
    ...
    const getSrcSet = (url: string) => {
        ...
        const baseUrl = url.split('?')[0];
        return [400, 800, 1200, 1600, 2000].map(w =>
            `${baseUrl}?auto=format&q=75&fit=crop&w=${w} ${w}w`
        ).join(', ');
    };
    ...
    return (
        <div
            className={cn(
                "relative overflow-hidden bg-muted/10 transition-colors duration-500",
                containerClassName
            )}
        >
            ...
            <AnimatePresence>
                {!isLoaded && !error && (
                    <motion.div
                        ...
                        className="absolute inset-0 bg-muted/5"
                    />
                )}
            </AnimatePresence>
            ...
            <motion.img
                ...
                alt={alt}
                initial={blur ? { filter: "blur(20px)", opacity: 0, scale: 1.05 } : { opacity: 0 }}
                animate={
                    isLoaded
            ...
            {error && (
                <div className="absolute inset-0 flex flex-col items-center justify-center bg-muted/10 text-muted-foreground gap-2">
