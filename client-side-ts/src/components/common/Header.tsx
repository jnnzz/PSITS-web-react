...
export const Header = () => {
  ...
  return (
    <div className={cn("z-50 flex justify-center px-3 md:px-0 left-0 right-0 top-2 md:top-4", isStaticPage ? "absolute" : "fixed")}>
      <motion.nav
        variants={{
      ...
      >
        ...
        <Link to="/" className="flex items-center gap-3 shrink-0 group">
          ...
          <span className="lg:hidden font-bold text-lg text-gray-900">PSITS</span>
        </Link>
        ...
        <div className="flex items-center gap-2 md:gap-4">
          ...
          {/* Join Us Button */}
          <Button asChild className="rounded-full h-9 px-4 md:px-5 bg-[#1c9dde] hover:bg-[#1c9dde]/90 text-white font-semibold shadow-md active:scale-95 transition-all">
              ...
              <div className="flex flex-col p-4 gap-2">
                {navLinks.map((link) => (
                  ...
                  >
                    ...
                    {link.hasDropdown && <ChevronDown size={18} className={cn("transition-transform duration-200", "opacity-50")} />}
                  </NavLink>
                ))}
                <div className="h-px bg-gray-100 my-2 mx-2" />
                <div className="flex flex-col gap-2">
                  <Button asChild variant="ghost" className="w-full rounded-2xl py-6 text-base font-semibold">
                    <Link to="/auth/login" onClick={() => setIsOpen(false)}>Sign in</Link>
                  </Button>
                  <Button asChild className="w-full rounded-2xl py-6 text-base font-bold bg-[#1c9dde] shadow-xl">
                    <Link to="/auth/signup" onClick={() => setIsOpen(false)}>Join Us</Link>
                  </Button>
                </div>
              </div>
            </motion.div>
