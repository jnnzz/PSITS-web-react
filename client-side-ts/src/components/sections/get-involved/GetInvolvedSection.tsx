      ...
      <div className={cn('bg-background overflow-x-clip', isLg ? 'sticky top-0 h-screen overflow-y-hidden' : 'py-16 md:py-20')}>
        ...
        <div className="container px-4 md:px-6">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-start">
                ...
                <div className="flex items-center gap-3 md:gap-4 pt-2">
                  <div className="flex -space-x-2 md:-space-x-3">
                    {[1, 2, 3, 4].map((i) => (
                      <div 
                        key={i} 
                        className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-muted border-2 border-background" 
                      />
                    ))}
                  </div>
            ...
            {/* Right Column - Cards (Scrollable on desktop) */}
            <div className="lg:col-span-7">
              <div 
                ref={cardsRef} 
                className="flex flex-col gap-4 md:gap-6" 
                style={isLg ? { transform: `translateY(-${scrollY}px)` } : undefined}
              >
                {getInvolvedData.cards.map((card, i) => (
                  <GetInvolvedCard key={i} {...card} />
                ))}
              </div>
              <p className="flex items-center text-sm font-medium text-muted-foreground mt-6">
