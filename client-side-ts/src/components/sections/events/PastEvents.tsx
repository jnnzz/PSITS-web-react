    ...
    return (
        <section className="py-20 px-4 md:px-8 max-w-7xl mx-auto">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-16 gap-6">
                <div>
                    <h2 className="text-4xl md:text-5xl font-black text-foreground tracking-tight">{header.title}</h2>
                    <p className="text-2xl font-bold text-primary mt-2">{header.year}</p>
                </div>
                <Button variant="outline" className="rounded-full flex items-center gap-2 text-muted-foreground bg-gray-100/40 border-gray-200 px-6 py-6 font-semibold">
            ...
            <div className="space-y-20">
                {events.map((event) => (
                    <div key={event.id} className="flex flex-col md:flex-row gap-8 md:gap-12 group">
                        {/* Date column */}
                        <div className="flex flex-col items-start md:items-center justify-start min-w-[120px] shrink-0">
                            <span className="text-base font-bold text-muted-foreground/80 uppercase tracking-widest mb-1">{event.date.month}</span>
                            <span className="text-6xl md:text-7xl font-black text-foreground leading-[0.8]">{event.date.day}</span>
                        </div>
